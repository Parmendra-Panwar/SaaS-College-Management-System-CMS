...
const TeacherDashboard = () => {
    const [activeTab, setActiveTab] = useState('attendance'); // 'attendance', 'grades', 'discipline'
    ...
    return (
        <div className="max-w-[1305px] mx-auto px-6 py-10 w-full animate-in fade-in slide-in-from-bottom-4 duration-500 bg-[#FDFCF0] min-h-screen">
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
                <div className="flex border-b mb-6 border-gray-200 gap-6">
                    <button onClick={() => setActiveTab('attendance')} className={`pb-3 font-bold text-lg transition-colors ${activeTab === 'attendance' ? 'border-b-4 border-indigo-600 text-indigo-600' : 'text-gray-400 hover:text-gray-700'}`}>Take Attendance</button>
                    <button onClick={() => setActiveTab('grades')} className={`pb-3 font-bold text-lg transition-colors ${activeTab === 'grades' ? 'border-b-4 border-indigo-600 text-indigo-600' : 'text-gray-400 hover:text-gray-700'}`}>Input Grades</button>
                    <button onClick={() => setActiveTab('discipline')} className={`pb-3 font-bold text-lg transition-colors ${activeTab === 'discipline' ? 'border-b-4 border-indigo-600 text-indigo-600' : 'text-gray-400 hover:text-gray-700'}`}>Discipline / Notes</button>
                </div>
